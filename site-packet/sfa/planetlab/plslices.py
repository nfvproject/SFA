...
class PlSlices:
    ...
    def verify_site(self, slice_xrn, slice_record=None, sfa_peer=None, options=None):
        ...
        sites = self.driver.shell.GetSites({'peer_id': None},['site_id','name','abbreviated_name','login_base'])

        # filter sites by hrn
        #wangyang
        #site_exists = [site for site in sites if site['hrn'] == site_hrn]
        site_exists = [site for site in sites if (top_auth_hrn+'.'+site['login_base']) == site_hrn]
        #site_exists['hrn'] = site_hrn
        if not site_exists:
    ...
    def verify_slice(self, slice_hrn, slice_record, sfa_peer, expiration, options=None):
        ...
        #login_base = plxrn.pl_login_base()
        #wangyanglogger
        #logger.log_exc('wangyangg:slice_hrn is %s' %slice_hrn)
        #logger.log_exc('wangyangg:slice_name is %s' %slicename)
        slices = self.driver.shell.GetSlices({'peer_id': None},['slice_id','name','hrn'])
        
        #wangyang
        #wangyanglogger
        #for slice in slices:
        #    logger.log_exc('wangyang:slice[hrn] is %s' %slice['hrn'])
        #    logger.log_exc('wangyang:slice[name] is %s' %slice['name'])
        # Filter slices by HRN
        #slice_exists = [slice for slice in slices if slice['hrn'] == slice_hrn]
        slice_exists = [slice for slice in slices if slice['name'] == slice_name]
        ...
        if not slice_exists:
            ...
            slice = slice_exists[0]
             #wangyang 
            slice['hrn'] = top_auth_hrn+"."+login_base+"."+slice_part
    ...
    def verify_persons(self, slice_hrn, slice_record, users, sfa_peer, options=None):
        ...
        sites = self.driver.shell.GetSites({'peer_id': None}, ['node_ids', 'site_id', 'name', 'person_ids', 'slice_ids', 'login_base'])
        #wangyanglogger
        #for site in sites:
        #    logger.log_exc('wangyang in verify_person:site[name] is %s' %(top_auth_hrn+'.'+site['name']))
        #    logger.log_exc('wangyang in verify_person:site_name is %s' %site_hrn)
        site = [site for site in sites if (top_auth_hrn+'.'+site['name']) == site_hrn][0]
        slices = self.driver.shell.GetSlices({'peer_id': None}, ['slice_id', 'node_ids', 'person_ids', 'expires', 'site_id', 'name', 'hrn'])
        slice = [slice for slice in slices if slice['name'] == slice_name][0]
